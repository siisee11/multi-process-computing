...
{
    clock_t in_start, in_finish, ge_start, ge_finish, bs_start, bs_finish;
    ...
    in_finish = ge_start = clock();
    if (!triangular_mode) {
        gaussian_elimination();
    }
    ge_finish = bs_start = clock();
...
#   endif
    ...
    printf("Nthreads=%2d  ERR=%8.1e  INIT: %8.4fs  GAUS: %8.4fs  BSUB: %8.4fs\n",
            1, find_max_error(),
            (float)(in_finish-in_start)/CLOCKS_PER_SEC,
            (float)(ge_finish-ge_start)/CLOCKS_PER_SEC,
            (float)(bs_finish-bs_start)/CLOCKS_PER_SEC);
