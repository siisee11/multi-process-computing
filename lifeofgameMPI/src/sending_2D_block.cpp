...
int main(int argc, char **argv) {
	int **global, **local;
	const int gridsize=16; // size of grid
	const int procgridsize=8;  // size of process grid
	...
	int sizes[2]    = {16, 16};         /* global size */
	int subsizes[2] = {8, 8};
	int starts[2]   = {0,0};                        /* where this one starts */
	MPI_Datatype type, subarrtype;
	MPI_Type_create_subarray(2, sizes, subsizes, starts, MPI_ORDER_C, MPI_INT, &type);
	MPI_Type_create_resized(type, 0, 8*sizeof(int), &subarrtype);
