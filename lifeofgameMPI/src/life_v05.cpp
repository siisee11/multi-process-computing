...
int doCalc(int **array, int **newarray, int rs, int rf, int cs, int cf, int move, int skip){
	int sum = 0;
	for (int x = rs + skip; x < rf - skip; x++){
		for (int y = cs + skip; y < cf - skip; y++){
			for(int i = -1; i <= 1; i++){
				for(int j = -1; j <=1; j++){
					if(i!=0||j!=0)
...
int main(int argc, char* argv[])
{
	...
	/* row col split */
	char name[100], rname[100], cname[100];
	int cnt, rlen;
	...
	N = data[0];	generations = data[1];	nghosts = data[2];
...
*/
	...
	MPI_Datatype col_side_comm_slice, row_side_comm_slice;
	sizes[0]= row_per_proc;		sizes[1]= col_per_proc;
	subsizes[0]= row_per_proc;	subsizes[1]= 1+nghosts;
	starts[0]= 0;				starts[1]= 0;
	MPI_Type_create_subarray(2, sizes, subsizes, starts, MPI_ORDER_C, MPI_INT, &col_side_comm_slice);
	MPI_Type_commit(&col_side_comm_slice);
	subsizes[0]= 1+nghosts;		subsizes[1]= col_per_proc;
	MPI_Type_create_subarray(2, sizes, subsizes, starts, MPI_ORDER_C, MPI_INT, &row_side_comm_slice);
	MPI_Type_commit(&row_side_comm_slice);
	...
	{	
		 ...
		 * +--------------------------------------------------------+
		 * | each process is in their own row and col communities.	|
		 * | Row side communication first.							|
		 * | Col side communication second.							|
		 * +--------------------------------------------------------+
		 */
		...
		if (g%(nghosts+1)==0){
			...
			if (my_row_rank!=0){
				MPI_Isend(&(myslice[0][1+nghosts]), 1, col_side_comm_slice, my_row_rank-1, 1, comm1D_row, &req[0]);
				MPI_Irecv(&(myslice[0][0]), 1, col_side_comm_slice, my_row_rank-1, 1, comm1D_row, &req[1]);
			}

			if (my_row_rank!=ncols-1){
				MPI_Isend(&(myslice[0][col_per_proc-2*(1+nghosts)]), 1, col_side_comm_slice, my_row_rank+1, 1, comm1D_row, &req[2]);
				MPI_Irecv(&(myslice[0][col_per_proc-(1+nghosts)]), 1, col_side_comm_slice, my_row_rank+1, 1, comm1D_row, &req[3]);
			}

			/* Calculate Inner part of myslice that is independent with outer cells. */
			doCalc(myslice, mynewslice, 0, half, 0, real_col-2, 2+nghosts, 0);
			...
			if (my_col_rank!=0){
				MPI_Isend(&(myslice[1+nghosts][0]), 1, row_side_comm_slice, my_col_rank-1, 1, comm1D_col, &req[0]);
				MPI_Irecv(&(myslice[0][0]), 1, row_side_comm_slice, my_col_rank-1, 1, comm1D_col, &req[1]);
			}

			if (my_col_rank!=nrows-1){
				MPI_Isend(&(myslice[row_per_proc-2*(1+nghosts)][0]), 1, row_side_comm_slice, my_col_rank+1, 1, comm1D_col, &req[2]);
				MPI_Irecv(&(myslice[row_per_proc-(1+nghosts)][0]), 1, row_side_comm_slice, my_col_rank+1, 1, comm1D_col, &req[3]);
			}

			/* Calculate Inner part of myslice that is independent with outer cells. */
			doCalc(myslice, mynewslice, half, real_row-2, 0, real_col-2, 2+nghosts, 0);
			...
			doCalc(myslice, mynewslice, 1, 2+nghosts, 1+nghosts, col_per_proc-1-nghosts, 0, 0);
			doCalc(myslice, mynewslice, row_per_proc-2-nghosts, row_per_proc-1, 1+nghosts, col_per_proc-1-nghosts, 0, 0);
			doCalc(myslice, mynewslice, 1, row_per_proc-1, 1, nghosts+2, 0, 0);
			doCalc(myslice, mynewslice, 1, row_per_proc-1, col_per_proc-2-nghosts, col_per_proc-1, 0, 0);
		 	/* end of calculation of rest part. */

		}	/* end of communication part */	
		else
		{
			doCalc(myslice, mynewslice, 1, row_per_proc-1, 1, col_per_proc - 1, 0, g%(nghosts+1));
		}
	...
	MPI_Type_free(&col_side_comm_slice);
	MPI_Type_free(&row_side_comm_slice);
